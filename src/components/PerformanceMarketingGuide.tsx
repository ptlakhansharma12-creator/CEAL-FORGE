...
const pmBenefits: PMBenefit[] = [
  ...
  },
  {
    ...
    image: '/pm-roas-attribution.jpg',
    metrics: [
      { label: 'ROAS Target', val: '6.85x' },
      { label: 'Attribution Speed', val: '< 1 Sec' },
      { label: 'Dashboard Access', val: '24/7 Live' }
    ],
    borderGlow: 'hover:border-cyan-500/60 shadow-[0_0_25px_rgba(56,189,248,0.2)]'
  },
  {
    id: 'cac-reduction',
    num: '03',
...
export function PerformanceMarketingGuide() {
  ...
  return (
    <section id="performance-guide" className="py-20 sm:py-32 bg-[#070A12] text-white relative overflow-hidden select-none border-y border-slate-800/80">
      ...
      <div className="absolute bottom-0 right-0 w-[600px] h-[600px] bg-cyan-600/10 rounded-full blur-[180px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center mb-16">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-xs font-mono font-extrabold ${isSelected ? 'text-cyan-400' : 'text-slate-500'}`}>
                      0{idx + 1}
                    </span>
                  </div>
          ...
          {/* Right Column: Featured Large Card with Image (7 cols) */}
          <motion.div
          ...
          >
            {/* Image Preview Box */}
            <div className="w-full h-64 sm:h-80 rounded-2xl overflow-hidden relative mb-6 border border-slate-800 bg-black/60">
              <img
              ...
              <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between z-10">
                <span className="text-xs font-mono font-bold text-cyan-300 bg-cyan-950/80 border border-cyan-500/60 px-3 py-1 rounded-full uppercase backdrop-blur-md">
                  ⚡ {currentBenefit.badge}
                </span>
                <span className="text-xs font-mono text-purple-300 bg-purple-950/80 border border-purple-700 px-3 py-1 rounded-full backdrop-blur-md">
                  CLIENT BENEFIT {currentBenefit.num}
                </span>
              </div>
            </div>
              ...
              <div className="grid grid-cols-3 gap-3 pt-4 border-t border-slate-800 bg-slate-950/60 rounded-2xl p-3 text-center border">
                {currentBenefit.metrics.map((m, i) => (
                  <div key={i}>
                    ...
                    <div className="text-sm sm:text-base font-extrabold text-white font-mono mt-0.5">{m.val}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* 4 Image Summary Grid */}
        <div className="mt-16 pt-16 border-t border-slate-800">
          <div className="text-center max-w-2xl mx-auto mb-10">
            <h3 className="text-xl sm:text-3xl font-extrabold text-white font-heading">
              The 4 Pillars of Performance Marketing Value
            </h3>
            <p className="text-xs sm:text-sm text-slate-400 mt-2">
              Visualizing how performance marketing replaces unmeasured spend with scalable revenue.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {pmBenefits.map((b) => (
              <div key={b.id} className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 hover:border-purple-500/50 transition-all group">
                <div className="w-full aspect-square rounded-xl overflow-hidden mb-3 border border-slate-800 relative">
                  <img src={b.image} alt={b.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  <div className="absolute top-2 left-2 text-[10px] font-mono font-bold text-white bg-slate-950/80 px-2 py-0.5 rounded">
                    {b.num}
                  </div>
                </div>
        ...
        {/* Bottom CTA Banner */}
        <div className="mt-16 p-8 sm:p-10 rounded-3xl bg-gradient-to-r from-purple-950/80 via-slate-900 to-indigo-950/80 border border-purple-500/40 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-[0_20px_50px_rgba(168,85,247,0.15)]">
          <div>

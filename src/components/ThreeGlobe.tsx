...
export function ThreeGlobe({ className = '' }: ThreeGlobeProps) {
  ...
  useEffect(() => {
    ...
    // 1. Globe Mesh (Wireframe Sphere in Electric Neon Blue)
    ...
    // 2. Inner Glowing Core (Cyan / Deep Blue)
    ...
    const ringGeo = new THREE.RingGeometry(0.25, 0.5, 32);
    ...
    // 4. Orbiting Cyber Ring 1 (Electric Blue)
    const orbit1Geo = new THREE.TorusGeometry(6.2, 0.03, 16, 100);
    const orbit1Mat = new THREE.MeshBasicMaterial({ color: 0x3b82f6, transparent: true, opacity: 0.5 });
    const orbit1Mesh = new THREE.Mesh(orbit1Geo, orbit1Mat);
    orbit1Mesh.rotation.x = Math.PI / 3;
    mainGroup.add(orbit1Mesh);

    // 5. Orbiting Cyber Ring 2 (Cyan Glow)
    const orbit2Geo = new THREE.TorusGeometry(7.2, 0.02, 16, 100);
    const orbit2Mat = new THREE.MeshBasicMaterial({ color: 0x38bdf8, transparent: true, opacity: 0.4 });
